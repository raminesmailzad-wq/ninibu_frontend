...
import { Badge, Button, Card, ChoiceModal, EmptyState, ErrorState, FormModal, Header, Loading, Screen, SectionTitle, SegmentedControl } from '@/components/ui';
import { colors, typography } from '@/theme';
import { SponsoredSlot } from '@/components/SponsoredSlot';
import { useChild } from '@/providers/ChildProvider';
import { useSession } from '@/providers/SessionProvider';

type Tab = 'smart' | 'knowledge' | 'search' | 'care';
const careKinds = [
  { id: 1, value: '', label: 'همه مراکز' }, { id: 2, value: 'health_center', label: 'مرکز سلامت' }, { id: 3, value: 'doctor_office', label: 'مطب پزشک' }, { id: 4, value: 'clinic', label: 'کلینیک' }, { id: 5, value: 'hospital', label: 'بیمارستان' }, { id: 6, value: 'laboratory', label: 'آزمایشگاه' }, { id: 7, value: 'dental_clinic', label: 'دندانپزشکی' }, { id: 8, value: 'rehabilitation_center', label: 'توانبخشی' }, { id: 9, value: 'maternity_hospital', label: 'زنان و زایمان' },
];
...
export default function Discover() {
  ...
  return <Screen refreshing={loading} onRefresh={load}>
    ...
    <SponsoredSlot placement="public_content_list" />
    <SegmentedControl<Tab> value={tab} onChange={setTab} items={[{ value: 'smart', label: 'برای شما' }, { value: 'knowledge', label: 'دانشنامه' }, { value: 'search', label: 'جست‌وجو' }, { value: 'care', label: 'مراکز' }]} />
    {error ? <ErrorState message={error} onRetry={load} /> : loading ? <Loading /> : tab === 'smart' ? <SmartFeed data={personal} onOpen={setDetailSlug} onReload={load} /> : tab === 'knowledge' ? <Knowledge data={contents} categories={categories} bookmarks={bookmarks} onOpen={setDetailSlug} /> : tab === 'search' ? <SearchPanel onOpen={setDetailSlug} onOpenCare={(title) => { setCareSearchSeed(title); setTab('care'); }} /> : <CarePanel cityId={profile?.city?.id} cityName={profile?.city?.local_name || profile?.city?.name} initialSearch={careSearchSeed} />}
    <KnowledgeDetailModal slug={detailSlug} bookmarks={bookmarks} onClose={() => setDetailSlug(undefined)} onBookmarkChanged={load} />
  </Screen>;
}

function SmartFeed({ data, onOpen, onReload }: { data?: PersonalizationFeedResponse; onOpen: (slug: string) => void; onReload: () => Promise<void> }) { async function feedback(id: number, type: 'helpful' | 'not_interested') { await api(apiPaths.personalizationFeedback(id), { method: 'POST', body: JSON.stringify({ feedback_type: type }) }); await onReload(); } return <><SectionTitle title="پیشنهادهای شخصی" />{data?.items.length ? data.items.map((item) => <Card key={item.recommendation_id}><View style={s.itemTop}><Badge tone="purple">پیشنهاد نینیبو</Badge><Ionicons name="sparkles" size={19} color={colors.primary} /></View><Text style={s.title}>{item.title}</Text>{item.summary ? <Text style={s.copy}>{item.summary}</Text> : null}<View style={s.actions}>{item.entity_type === 'knowledge_content' && typeof item.metadata?.slug === 'string' ? <Button compact title="مشاهده" variant="secondary" onPress={() => onOpen(String(item.metadata?.slug))} /> : <View />}<View style={s.feedback}><Pressable onPress={() => void feedback(item.recommendation_id, 'helpful')}><Ionicons name="thumbs-up-outline" size={19} color={colors.primary} /></Pressable><Pressable onPress={() => void feedback(item.recommendation_id, 'not_interested')}><Ionicons name="thumbs-down-outline" size={19} color={colors.muted} /></Pressable></View></View></Card>) : <EmptyState title="فعلاً پیشنهاد تازه‌ای نداریم" />}</>; }

function Knowledge({ data, categories, bookmarks, onOpen }: { data?: KnowledgeContentListResponse; categories: KnowledgeCategory[]; bookmarks: KnowledgeContent[]; onOpen: (slug: string) => void }) { const [category, setCategory] = useState(''); const [items, setItems] = useState<KnowledgeContent[]>(data?.items ?? []); const [busy, setBusy] = useState(false); useEffect(() => { setItems(data?.items ?? []); }, [data]); async function choose(next: string) { setCategory(next); setBusy(true); try { const params = new URLSearchParams({ language: 'fa', page: '1', limit: '30' }); if (next) params.set('category', next); const result = await api<KnowledgeContentListResponse>(`${apiPaths.content}?${params.toString()}`); setItems(result.items); } finally { setBusy(false); } } const bookmarkIds = new Set(bookmarks.map((item) => item.id)); return <><SectionTitle title="مطالب آموزشی برای والدین" />{categories.length ? <View style={s.chips}><Pressable onPress={() => void choose('')} style={[s.chip, !category && s.chipActive]}><Text style={s.chipText}>همه</Text></Pressable>{categories.slice(0, 8).map((cat) => <Pressable key={cat.id} onPress={() => void choose(cat.code)} style={[s.chip, category === cat.code && s.chipActive]}><Text style={s.chipText}>{cat.name}</Text></Pressable>)}</View> : null}{busy ? <Loading /> : items.length ? items.map((item) => <Pressable key={item.id} onPress={() => onOpen(item.slug)}><Card><View style={s.itemTop}><Badge tone={item.medical_review_required ? 'pink' : 'gray'}>{contentType(item.content_type)}</Badge>{bookmarkIds.has(item.id) ? <Ionicons name="bookmark" size={18} color={colors.primary} /> : <Text style={s.date}>{formatJalaliShortDate(item.published_at || item.updated_at)}</Text>}</View><Text style={s.title}>{item.title}</Text>{item.summary ? <Text style={s.copy}>{item.summary}</Text> : null}</Card></Pressable>) : <EmptyState title="محتوایی برای نمایش نیست" />}</>; }
