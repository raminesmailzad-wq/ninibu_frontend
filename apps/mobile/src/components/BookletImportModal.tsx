...
import { BookletCameraModal } from '@/components/BookletCameraModal';
...
export function BookletImportModal({ visible, childId, childName, birthDate, onClose, onSaved }: { visible: boolean; childId: number; childName: string; birthDate: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const selectedPage = useMemo(() => pages.find((page) => page.value === pageType) ?? pages[0]!, [pageType]);
  const childAgeMonths = useMemo(() => completedAgeMonths(birthDate), [birthDate]);

  useEffect(() => {
    if (!visible) {
      setPageType('weight_for_age'); setAsset(null); setCameraOpen(false); setResult(null); setRows([]); setBusy(false); setError(''); setPhotoWarning('');
    }
  }, [visible]);

  function resetPhoto() {
    setAsset(null); setResult(null); setRows([]); setError(''); setPhotoWarning('');
  }
  ...
  async function choosePhoto() {
    setError(''); setPhotoWarning('');
    const picked = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], quality: 0.95, allowsEditing: false });
  ...
  function rowsFromResult(data: DocumentImport): ReviewRow[] {
    const currentAge = childAgeMonths;
...
const s = StyleSheet.create({
  ...
  captureGuide: { minHeight: 190, borderWidth: 1, borderStyle: 'dashed', borderColor: colors.border, borderRadius: 18, alignItems: 'center', justifyContent: 'center', padding: 20, gap: 7, backgroundColor: colors.mutedBackground },
  phoneGuide: { width: 126, height: 84, borderWidth: 2, borderColor: colors.primary, borderRadius: 15, padding: 8, alignItems: 'center', justifyContent: 'center', position: 'relative' },
  pageGuide: { width: 88, height: 56, borderWidth: 2, borderStyle: 'dashed', borderColor: colors.primary, borderRadius: 5, alignItems: 'center', justifyContent: 'center' },
  ...
  ageText: { fontFamily: typography.regular, fontSize: 10, color: colors.muted, textAlign: 'right', writingDirection: 'rtl', lineHeight: 18 },
