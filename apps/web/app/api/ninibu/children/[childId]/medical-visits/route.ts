...
export async function GET(req: Request, { params }: { params: Promise<{ childId: string }> }) {
  ...
  const result = await authorizedBackend(`${apiPaths.childMedicalVisits(childId)}${new URL(req.url).search}`);
  return NextResponse.json(result.body, { status: result.status });
}
export async function POST(req: Request, { params }: { params: Promise<{ childId: string }> }) {
  const { childId } = await params;
  const result = await authorizedBackend(apiPaths.childMedicalVisits(childId), { method: "POST", body: JSON.stringify(await req.json()) });
  return NextResponse.json(result.body, { status: result.status });
}
